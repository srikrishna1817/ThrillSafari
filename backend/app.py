...
class ParkModel:
    ...
    def __init__(self, total_time, max_daily_rides, is_vip, bad_weather, user_age=25, user_weight=70):
        self.rides = []
        self.ride_count = 0
        self.total_time = total_time
        self.max_daily_rides = max_daily_rides
        self.is_vip = is_vip
    ...
    def add_default_rides(self):
        ...
        if not self.rides:
            ...
            # Land Rides (from image, with defaults)
            # Land Rides with realistic Wonderla thrill levels and constraints
            land_rides_data = [
                ("L001", "Mission Interstellar", 9, 3, 25, 8, False, True, True, True, "land", 40, 120, 12, 65),
                ...
                ("L004", "Twist and Shout", 7, 3, 20, 6, False, True, True, True, "land", 35, 110, 10, 60),
                ...
                ("L007", "Pirate Ship", 8, 4, 18, 7, False, True, True, True, "land", 40, 130, 12, 65),
                ...
                ("L011", "Sky Tilt (New)", 8, 3, 22, 7, False, True, True, True, "land", 45, 120, 14, 60),
                ("L012", "Hyperverse", 10, 2, 30, 9, False, True, True, True, "land", 50, 110, 16, 55),
                ("L013", "Recoil", 9, 3, 28, 8, False, True, True, True, "land", 45, 120, 14, 60),
                ("L014", "Maverick", 9, 3, 25, 8, False, True, True, True, "land", 45, 115, 14, 65),
                ("L015", "Equinox", 8, 4, 20, 7, False, True, True, True, "land", 40, 120, 12, 65),
                ("L016", "Techno Jump", 7, 2, 15, 6, False, True, True, True, "land", 35, 110, 10, 70),
                ("L017", "Twin Flip T Rex", 8, 3, 22, 7, False, True, True, True, "land", 40, 120, 12, 65),
            ...
            # Water Rides (newly added with W-prefix and default values)
            # Water Rides with realistic constraints
            water_rides_data = [
                ("W001", "Rainbow Loooops", 8, 4, 25, 7, False, True, True, True, "water", 40, 120, 12, 65),
                ("W002", "Drop Loop", 9, 3, 30, 8, False, True, True, True, "water", 45, 110, 14, 60),
                ("W003", "Rain Disco", 6, 5, 15, 5, False, False, True, True, "water", 25, 120, 8, 75),
                ("W004", "Boomerang", 7, 4, 20, 6, False, True, True, True, "water", 35, 115, 10, 70),
                ...
                ("W008", "Bullet", 8, 2, 25, 7, False, True, True, True, "water", 40, 115, 12, 65),
                ("W009", "Wavy and Vertical Fall", 9, 3, 28, 8, False, True, True, True, "water", 45, 120, 14, 60),
                ("W010", "Harakiri", 10, 2, 35, 9, False, True, True, True, "water", 50, 110, 16, 55),
                ("W011", "Mammoth", 7, 5, 18, 6, False, True, True, True, "water", 35, 130, 10, 70),
                ...
                ("W016", "Drop and Tornado", 8, 4, 22, 7, False, True, True, True, "water", 40, 120, 12, 65),
                ("W017", "Screw", 7, 3, 18, 6, False, True, True, True, "water", 35, 115, 10, 70)
            ]

            # Kids Rides with age-appropriate constraints
            kids_rides_data = [
                ...
                ("K011", "Mini Top Spin", 3, 3, 10, 2, False, False, False, False, "kids", 18, 85, 5, 16),
...
class PlanModel:
    ...
    def __init__(self, selected_rides, total_thrill, remaining_time, remaining_fatigue):
        self.selected_rides = selected_rides
        self.selected_count = len(selected_rides)
    
        self.total_thrill = total_thrill
        self.remaining_time = remaining_time
        self.remaining_fatigue = remaining_fatigue

# --- Heap Implementation ---
# --- Heap Implementation ---
def generate_optimal_plan(model):
    """
    Generates an optimal ride plan based on available time, max daily rides,
    ...
    NEW: Supports ride preference (dry_only, wet_only, or mixed with dry first).
    """
    heap = []  # Min-heap: store negative thrill for max-heap behavior
    plan = PlanModel([], 0, model.total_time, model.max_daily_rides) # remaining_fatigue now represents max_daily_rides
    ...
    # NEW: Separate rides by type if ride_preference is specified
    eligible_rides = []
    ...
    # NEW: Apply ride preference filtering and ordering
    ...
    if not hasattr(model, 'ride_preference') or not model.ride_preference or model.ride_preference not in ['dry_only', 'wet_only', 'dry_first']:
        ...
        # Iterate while there are rides in the heap and max daily rides limit is not reached
        while heap and len(plan.selected_rides) < model.max_daily_rides:
        ...
        # NEW: Sequential selection for dry_first, dry_only, or wet_only
        for i, ride in eligible_rides:
            if len(plan.selected_rides) >= model.max_daily_rides:
                break
    ...
    # After the loop, plan.remaining_fatigue should reflect the actual remaining ride slots
    plan.remaining_fatigue = model.max_daily_rides - len(plan.selected_rides)

    return plan

# --- Global ParkModel Instance ---
initial_total_time = 180
initial_max_daily_rides = 8
initial_is_vip = True
initial_bad_weather = False
initial_user_age = 25
initial_user_weight = 70
park_model = ParkModel(initial_total_time, initial_max_daily_rides, initial_is_vip, initial_bad_weather, initial_user_age, initial_user_weight)
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            data = jwt.decode(token, app.config['SECRET_SECRET_KEY'], algorithms=["HS256"])
...
def generate_plan():
    ...
    try:
        ...
        if 'total_time' not in data or 'max_daily_rides' not in data or 'user_age' not in data or 'user_weight' not in data:
            return jsonify({'error': 'total_time, max_daily_rides, user_age, and user_weight are required'}), 400

        # Update park model parameters with validation
        total_time = int(data.get('total_time'))
        max_daily_rides = int(data.get('max_daily_rides'))
        ...
        park_model.max_daily_rides = max_daily_rides
        ...
        print(f"Updated ParkModel: total_time={park_model.total_time}, max_daily_rides={park_model.max_daily_rides}, is_vip={park_model.is_vip}, bad_weather={park_model.bad_weather}, user_age={park_model.user_age}, user_weight={park_model.user_weight}, ride_preference={park_model.ride_preference}")
        ...
        plan_data = {
            ...
            'remaining_rides': plan.remaining_fatigue,  # Now represents remaining ride slots
