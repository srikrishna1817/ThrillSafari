...
function App() {
  ...
  const [totalHours, setTotalHours] = useState(''); // New state for hours
  const [totalMinutes, setTotalMinutes] = useState(''); // New state for minutes
  const [maxDailyRides, setMaxDailyRides] = useState('');
  const [userAge, setUserAge] = useState('');
  const [userWeight, setUserWeight] = useState('');
  const [isVIP, setIsVIP] = useState(true);
  const [badWeather, setBadWeather] = useState(false);
  const [ridePreference, setRidePreference] = useState(''); // NEW: Ride preference state
  ...
  const [showAdminPanel, setShowAdminPanel] = useState(false); // NEW: Toggle admin panel visibility
  const isAdminLoggedIn = userRole === 'admin';
  // --- END NEW STATES ---

  // NEW: State for Kids Rides section visibility
  ...
  // Added 'token' to the dependency array to resolve the useEffect warning.
  // The effect will re-run if the token changes, ensuring userRole is always up-to-date.
  useEffect(() => {
    fetchRides();
    checkAPIHealth();
    // On mount, if a token exists, try to decode it to update userRole
    if (token) {
      try {
        // Simple client-side decode (not for security, just for UI state)
        const decodedToken = JSON.parse(atob(token.split('.')[1]));
        setUserRole(decodedToken.role || 'guest');
      } catch (e) {
        console.error("Error decoding token from localStorage:", e);
        handleLogout(); // Clear invalid token if decoding fails
      }
    }
 ...
 const validateNewRide = () => {
      ...
      // ID validation now checks for L, W, K prefixes
 ...
 // Handler for generating the optimal plan
  const handleGeneratePlan = async () => {
    ...
    // Parse hours and minutes, provide defaults if empty or invalid
    const parsedHours = parseInt(totalHours);
    const parsedMinutes = parseInt(totalMinutes);

    const finalHours = isNaN(parsedHours) || parsedHours < 0 ? 0 : parsedHours;
    const finalMinutes = isNaN(parsedMinutes) || parsedMinutes < 0 || parsedMinutes > 59 ? 0 : parsedMinutes;

    const totalTimeInMinutes = (finalHours * 60) + finalMinutes;

    // Validate total time
    if (totalTimeInMinutes <= 0) {
      setError('Total available time must be greater than 0 minutes.');
      return;
    }
    
    // Validate other required fields
    if (maxDailyRides === '' || userAge === '' || userWeight === '') {
      setError('Please enter Max Daily Rides, Your Age, and Your Weight before generating a plan.');
      return;
    }

    const parsedMaxDailyRides = parseInt(maxDailyRides);
    const parsedUserAge = parseInt(userAge);
    const parsedUserWeight = parseInt(userWeight);

    // Validate the parsed values
    if (isNaN(parsedMaxDailyRides) || parsedMaxDailyRides < 1) {
      setError('Max daily rides must be a positive number (at least 1).');
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/generate_plan`, {
        ...
        body: JSON.stringify({
          total_time: totalTimeInMinutes, // Send combined minutes
          max_daily_rides: parsedMaxDailyRides,
          user_age: parsedUserAge,
          user_weight: parsedUserWeight,
          is_vip: isVIP,
          bad_weather: badWeather,
          ride_preference: ridePreference, // NEW: Include ride preference
        }),
      });
      ...
      if (data.selected_rides && data.selected_rides.length > 0) {
        ...
        setError('No rides could be scheduled with the current preferences. Try increasing your time or fatigue limits, or adjusting age/weight criteria.');
      }
    } catch (err) {
  ...
  // NEW: Helper function to get ride type background color for plan display
  const getRideTypeColorClass = (type) => {
    switch(type) {
      case 'water':
        return 'table-info'; // Light blue background for water rides
      case 'land':
        return 'table-success'; // Light green background for land rides  
      case 'kids':
        return 'table-warning'; // Light yellow background for kids rides
      default:
        return '';
    }
  };

  // NEW: Helper function to get ride type icon
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setStaffIdInput('');
      setPasswordInput('');
      // The visibility is now handled by isAdminLoggedIn directly in render
    } catch (err) {
      console.error("Login failed:", err);
      setError(`Login failed: ${err.message}`);
      handleLogout(); // Ensure states are cleared on failed login
    } finally {
      setLoading(false);
    }
  };
  ...
  // --- END NEW ADMIN LOGIN / LOGOUT LOGIC ---
  ...
  const isKidUser = parseInt(userAge) < 13 && parseInt(userAge) >= 1; // Assuming age 1-12 is 'kid'
          ...
          <Col md={showAdminPanel ? 6 : 12}>
            <Card className="shadow-lg custom-card">
              <Card.Body>
                ...
                <Form>
                  <Row className="align-items-end"> {/* Align items to bottom to keep labels aligned */}
                    <Col xs={6} className="mb-3"> {/* Use mb-3 for consistent spacing */}
                      <Form.Group>
                        <Form.Label>Total Available Hours *</Form.Label>
                        <Form.Control
                          type="number"
                          value={totalHours}
                    ...
                    <Col xs={6} className="mb-3"> {/* Use mb-3 for consistent spacing */}
                      <Form.Group>
                        <Form.Label>Total Available Minutes *</Form.Label>
                        <Form.Control
                          type="number"
                          value={totalMinutes}
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Maximum Daily Rides *</Form.Label>
                  <Form.Control
                    type="number"
                    value={maxDailyRides}
                    onChange={(e) => setMaxDailyRides(e.target.value)}
                    min="1"
                    max="20"
                    placeholder="How many rides can you handle? (e.g., 8)"
                    className="custom-input"
                    required
                  />
                </Form.Group>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Button
                    ...
                    disabled={loading || maxDailyRides === '' || userAge === '' || userWeight === '' || (totalHours === '' && totalMinutes === '')}
                  >
                ...
                <Card.Body>
                  ...
                  {plan.selected_rides && plan.selected_rides.length > 0 ? (
                          ...
                          <tbody>
                            {plan.selected_rides.map((ride, index) => (
                              <tr key={index} className={getRideTypeColorClass(ride.type)}>
                                <td className="fw-bold">{index + 1}</td> {/* Removed text-dark */}
                                <td className="fw-bold">{ride.name}</td> {/* Removed text-dark */}
                                <td className="text-center"> {/* Removed text-dark */}
                                ...
                                <td>{ride.duration} min</td> {/* Removed text-dark */}
                                <td>{ride.queue_time} min</td> {/* Removed text-dark */}
                                <td className={plan.is_vip_used && ride.vip_queue_time < ride.queue_time ? 'text-success fw-bold' : ''}> {/* Removed text-dark */}
                                ...
                                <td className="fw-bold">{ride.duration + ride.vip_queue_time} min</td> {/* Removed text-dark */}
                              </tr>
                            ))}
                          </tbody>
                        </Table>
                      ...
                      <div className="mt-4 text-center text-light">
                        <Row className="g-3">
                          ...
                          <div className="p-3 bg-danger bg-opacity-25 rounded">
                            <h5 className="text-warning mb-1">Rides Left</h5>
                            <p className="fs-4 fw-bold mb-0">{plan.remaining_rides}</p>
                          </div>
                        </Col>
                        </Row>
                        ...
                        <Col md={2}>
                          <strong>Max Rides:</strong> {plan.max_daily_rides_used}
                        </Col>
                        <Col md={2}>
                    ...
                    <div className="text-center text-muted py-4">
                      ...
                      <p><small>Try increasing your time limit, fatigue limit, or adjusting other preferences.</small></p>
                    </div>
                  )}
                </Card.Body>
              </Card>
